...
def hypotheses(root):
 ...
 h1=card('카드 1 · 가격 범위 확대',bars([(r['label'],[(r['recovered'],r['n']),(r['detail'],r['n'])]) for r in d['condition_transitions']],['다음 검색 결과 회복','다음 검색 회복 + 상세 진입'])+f'<p><b>대상·행동: 가격 설정 사용자에게 최대가격 상향 또는 가격 필터 해제를 제안합니다.</b></p><p>가격 설정 검색의 원본 0건률은 106/146=72.6%였습니다. 기대효과는 검색 범위를 넓혀 SG3 재검색 회복을 높이고 SG4 지속 실패를 줄이는 것입니다. 성공 KPI는 가격 완화 후 회복률, SG3↑, SG4↓입니다. 가격 완화 단독 제안 효과는 아직 검증하지 않았습니다.</p>','합성 10,000명 · 2028-02-01~07 UTC · 분모: 0건 직후 다음 검색이 있는 전이 · 사용자 중복 포함','hypothesis-h1-chart')
 h2=card('카드 2 · 요구 옵션 수 감소',bars([(r['label']+'개',[(r['zero'],r['searches']),(r['detail'],r['searches'])]) for r in d['option_conversion']],['결과 없음 / 전체 검색','상세 진입 / 전체 검색'])+'<p><b>대상·행동: 옵션을 많이 설정한 사용자에게 필수 옵션을 단계적으로 줄이도록 제안합니다.</b></p><p>편의시설 3개 이상 검색의 원본 0건률은 120/136=88.2%였습니다. 기대효과는 핵심 선호를 유지하면서 후보 호텔을 늘려 SG3를 높이고 SG4를 줄이는 것입니다. 실제 옵션 의도와 제안 수용 로그가 없어 단독 효과는 미검증입니다.</p>','합성 10,000명 · 2028-02-01~07 UTC · 최초 옵션 수로 사용자군 고정, 이후 모든 검색 포함','hypothesis-h2-chart')
 h5=card('카드 3 · 인접지역 확대',bars([(r['label'],[(r['recovered'],r['n'])]) for r in d['city_transitions']],['지역 변경 직후 결과 회복'])+'<p><b>대상·행동: 위치 유연형에게 현재 5개 일본 도시 안의 인접·대체 지역을 제안합니다.</b></p><p>지역 변경 회복률 10/24=41.7%로 검색어 변경 30.0%, 조건 완화 26.8%보다 높게 관측됐습니다. 기대효과는 새로운 호텔 공급 후보를 노출해 지역 변경 후 회복률과 SG3를 높이는 것입니다. 24건의 소표본과 자기선택이 있어 제안 효과는 미검증입니다.</p>','원본 v03 · '+' ~ '.join(x[:19] for x in d['original_period'])+' KST · 분모: 0건 후 지역 변경 전이. 목적지 원문 비교 · 동일시각은 search_id 순.','hypothesis-h5-chart')
 statuses='<section class="panel" id="hypothesis-status"><h2>최신 우선순위 카드</h2><p><b>각 카드는 대상·개선 행동·성공 KPI를 함께 정의합니다.</b> 실제 기능별 A/B 효과는 아직 검증되지 않았습니다.</p><div class="scroll"><table><thead><tr><th>카드</th><th>대상과 개선</th><th>성공 KPI</th><th>선정 근거</th></tr></thead><tbody><tr><th>1 · 가격 범위 확대</th><td>가격 설정 사용자 · 최대가격 상향/필터 해제</td><td>SG3↑ · SG4↓ · 회복률</td><td>가격 설정 검색 0건률 106/146=72.6%</td></tr><tr><th>2 · 요구 옵션 수 감소</th><td>옵션 다수 설정 사용자 · 필수 옵션 단계적 해제</td><td>SG3↑ · SG4↓</td><td>편의시설 3개+ 검색 0건률 120/136=88.2%</td></tr><tr><th>3 · 인접지역 확대</th><td>위치 유연형 · 일본 5개 도시 인접/대체 지역</td><td>지역 변경 후 회복률 · SG3↑</td><td>지역 변경 후 회복 10/24=41.7%</td></tr></tbody></table></div><p class="meta">집계·관계 QA: 완료(PASS). 합성 기준선 교정: HOLD(16/20). 관측값은 카드 선정 근거이며 제안의 인과효과가 아닙니다.</p></section>'
 return '<h1>세 카드의 대상·개선 행동·성공 KPI를 확인합니다.</h1><p class="lead">가격 범위 확대, 요구 옵션 수 감소, 인접지역 확대를 분리해 무엇을 바꾸고 어떤 결과로 성공을 판단하는지 보여줍니다.</p>'+statuses+h1+h2+h5
