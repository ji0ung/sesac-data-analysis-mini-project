...
const slides=[
['먼저 무엇을 개선할 것인가?',4,['#template-story-topic']],
['가격·옵션 우선, 지역은 파일럿인 이유',4,['#template-story-problem']],
['32일차 검증 · 조인 증폭과 분모 확인',4,['#template-story-validation']],
...
['카드 1 · 가격 범위 확대',1,['#template-hypothesis-1']],
['카드 2 · 요구 옵션 수 감소',1,['#template-hypothesis-2']],
['카드 3 · 인접지역 확대',1,['#template-hypothesis-3']],
['검색의도 · 무엇을 바꿀 수 있나?',0,['#template-story-intents']],
['의도별 개선안 · 바꿀 수 있는 조건부터',2,['#template-story-proposals']],
['1만 명 합성 A/B · 실패·회복·상세진입',2,['#template-simulation_impacts']],
...
const agendaGroups=[['결론과 검증 근거',0,3],['전체 현황과 검색 흐름',3,6],['가설과 검증',6,9],['검색의도와 개선안',9,11],['합성 A/B와 결론',11,13],['데이터와 한계',13,15],['부록',15,18]];
...
function enterPresentation(index=0){document.body.classList.add('presenting');$('#presentation-mode').setAttribute('aria-pressed','true');$('#presentation-mode').textContent='대시보드로 돌아가기';document.querySelectorAll('details').forEach(d=>d.open=false);slideIndex=Number(index);renderSlide();$('#slide-title').focus({preventScroll:true});}
function togglePresentation(){if(document.body.classList.contains('presenting')){document.body.classList.remove('presenting');$('#presentation-mode').setAttribute('aria-pressed','false');$('#presentation-mode').textContent='슬라이드 보기';window.scrollTo(0,0);return;}enterPresentation(0);}
document.querySelectorAll('[data-slide-jump]').forEach(button=>button.onclick=()=>enterPresentation(button.dataset.slideJump));
