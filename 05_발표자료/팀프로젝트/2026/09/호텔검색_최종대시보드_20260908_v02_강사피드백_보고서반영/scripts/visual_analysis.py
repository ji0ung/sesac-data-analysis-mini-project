...
def hypotheses(root):
 ...
 h1=card('H1 · 조건 변경 후 결과 회복은 늘었지만, 상세 진입 개선은 확인되지 않았다.',bars([(r['label'],[(r['recovered'],r['n']),(r['detail'],r['n'])]) for r in d['condition_transitions']],['다음 검색 결과 회복','다음 검색 회복 + 상세 진입'])+f'<p><b>관측 차이: 변경군의 즉시 회복률은 유지군보다 {delta:+.2f}%p.</b></p><p>판독: 가격 또는 옵션 개수가 달라진 전이와 두 조건이 유지된 전이를 비교합니다. 유지군도 지역·검색어 등 다른 조건을 바꿀 수 있습니다. 자발적 행동과 생성 개입이 섞인 관측 비교로, 맞춤 조건 완화 제안의 인과효과는 판정 보류입니다.</p>','합성 10,000명 · 2028-02-01~07 UTC · 분모: 0건 직후 다음 검색이 있는 전이 · 사용자 중복 포함','hypothesis-h1-chart')
 h2=card('H2 · 초기 옵션 수에 따라 실패와 상세 진입이 다른가?',bars([(r['label']+'개',[(r['zero'],r['searches']),(r['detail'],r['searches'])]) for r in d['option_conversion']],['결과 없음 / 전체 검색','상세 진입 / 전체 검색'])+'<p>판독: 초기 조건이 엄격할수록 실패와 상세 진입이 어떻게 달라지는지 탐색합니다. 01·05에 가격·옵션 등 첫 재검색 행동 기반 추정군을 연결했습니다. 이 사후 분류 역시 6개 의도군의 효과 차이를 검증한 결과는 아닙니다. 의도 배정 후 각 의도 안의 A/B 차이를 비교해야 원래 가설을 판정할 수 있습니다.</p>','합성 10,000명 · 2028-02-01~07 UTC · 최초 옵션 수로 사용자군 고정, 이후 모든 검색 포함','hypothesis-h2-chart')
