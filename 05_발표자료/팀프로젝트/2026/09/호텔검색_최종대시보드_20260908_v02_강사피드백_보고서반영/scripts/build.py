...
import json
import sys
from exploratory import render_exploratory
from overview import render_overview
from visual_analysis import hypotheses
from concise_pages import ab_page,kpi_page
...
def render():
    ...
    values = {"DASHBOARD_CSS": css, "DASHBOARD_JS": code, "KPI_PAGE": (ROOT / "src/kpi.html").read_text(encoding="utf-8")}
    values.update({k:v for k,v in render_exploratory(ROOT).items() if k in ["EXPLORATORY_H1","EXPLORATORY_H2","EXPLORATORY_H5"]})
    values["AB_PAGE"] = ab_page(ROOT)
    values["OVERVIEW_DASHBOARD"] = render_overview(ROOT)
    values["HYPOTHESIS_CHARTS"] = hypotheses(ROOT)
    values["KPI_PAGE"] = kpi_page(ROOT)
