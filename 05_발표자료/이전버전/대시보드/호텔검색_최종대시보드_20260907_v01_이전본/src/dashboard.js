...
function page(i){document.querySelectorAll('.page').forEach(e=>e.hidden=e.id!=='page-'+i);document.querySelectorAll('nav button').forEach(e=>{const active=+e.dataset.page===i;e.setAttribute('aria-selected',active);e.tabIndex=active?0:-1});$('#slide-position').textContent=([4,0,1,2,3].indexOf(i)+1)+' / 5';window.scrollTo(0,0)}
...
$('#hyp-overview').innerHTML=[['H1 · A-1 전체 효과','0건 후 후속 검색의 회복률'],['H2 · A-1 의도별 차이','예산·옵션 수 의도군 안의 A/B 회복률'],['H5 · D-1 지역 확대','지역 변경 후속 검색 회복률·지역 선택률']].map(([title,metric])=>`<tr><td>${title}</td><td>${metric}</td><td>문서의 대조군·실험군 비교</td><td>A/B 결과 미연결</td></tr>`).join('');
const intents=[['예산 유연형','A-1 가격 범위 확대','H1/H2'],['옵션 유연형','A-1 필요한 옵션 수 감소','H1/H2'],['위치 유연형','D-1 인접지역 제안','H5'],['조건 고수형','F-1 반복 안내','추가 검토 · 문서 가설 제외'],['표현 수정형','D-2 연관 검색어 제안','추가 검토 · 발표 가설 제외'],['빠른 해결형','C-1 단계별 안내','추가 검토 · 발표 가설 제외']];$('#intent-content').innerHTML=intents.map(([name,proposal,hyp])=>`<article class="segment-card"><h3>${name}</h3><p><b>${proposal}</b></p><p class="mapping">${hyp}</p><p class="meta">실제 의도별 배정·A/B 결과 미연결</p></article>`).join('');
...
$('#findings').innerHTML=[['전체 세션의 즉시 회복',`회복 ${fmt(c.recovered)}/${fmt(c.followup)} = ${pct(c.recovered,c.followup)}%. 세션 마지막 검색 회복은 ${fmt(c.final_recovered)}/${fmt(c.zero_sessions)} = ${pct(c.final_recovered,c.zero_sessions)}%로 별도 지표입니다.`,'조건 완화의 기존 A/B 결과 확인'],['전체 방법별 회복 비교',`지역 변경 ${fmt(region.n)}/${fmt(region.d)} = ${pct(region.n,region.d)}%, 조건 완화 ${fmt(relax.n)}/${fmt(relax.d)} = ${pct(relax.n,relax.d)}%. 관측된 행동 차이입니다.`,'위치 유연형의 제안 효과와 대조'],['전체 동일 조건 반복',`동일 조건 반복의 회복 ${fmt(repeat.n)}/${fmt(repeat.d)} = ${pct(repeat.n,repeat.d)}%. 분모가 0이면 해당 전환이 없는 경우입니다.`,'반복 안내 후 반복률·회복률 확인']].map((x,i)=>`<article class="decision"><div class="big">0${i+1}</div><div><h2>${x[0]}</h2><p>${x[1]}</p></div><div class="next"><strong>연결할 판단</strong><p>${x[2]}</p></div></article>`).join('');
...
['상세진입 전환율은 73.8%가 아니라 19.5%이며, 반복조회 110건은 활동량으로 분리해야 합니다',0,['#template-card-h']],
['0건 검색 3,434건 중 95.3%가 재검색했지만, 즉시 회복은 17.1%에 그쳤습니다',0,['#template-story-funnel']],
...
['합성 확장은 분석 화면 검증에는 유용하지만 실제 A/B 효과를 증명하지 못합니다',4,['#template-data-limits']],
['[부록] 주 KPI와 보조 지표의 공식',4,['#primary-kpi','#supporting-kpis']],
['[부록] 목표 회복률을 건수로 환산하기',4,['#kpi-goal']],
...
function collapseSlideScript(){const host=$('#slide-script');if(!host.firstElementChild)return;const source=host.firstElementChild;const body=document.createElement('div');if(source.matches('details.speaker-notes')){[...source.childNodes].filter(node=>!(node.nodeType===1&&node.matches('summary'))).forEach(node=>body.append(node));}else{while(host.firstChild)body.append(host.firstChild);}const details=document.createElement('details');details.className='speaker-notes slide-script-toggle';const summary=document.createElement('summary');summary.textContent='발표 대본 · 클릭하여 펼치기';details.append(summary,body);host.replaceChildren(details);}
function renderSlide(){const [title,n,selectors]=slides[slideIndex];$('#slide-title').textContent=title;$('#slide-counter').textContent=(slideIndex+1)+' / '+slides.length;$('#slide-position').textContent=(slideIndex+1)+' / '+slides.length;$('#slide-stage').replaceChildren(...selectors.map(cloneForSlide));addFirstUseTermNotes();$('#slide-script').replaceChildren(cloneForSlide('#page-'+n+' > .speaker-notes'));if(selectors.includes('#primary-kpi')||selectors.includes('#kpi-goal')){const extra=document.createElement('p');extra.textContent='주 KPI는 0건 이후 후속 검색에서 결과를 찾은 비율입니다. 현재 확장본 기준 558/3,271 = 약 17.1%입니다. 모든 0건 검색을 분모로 하면 558/3,434 = 16.2%이므로 두 지표를 구분합니다. 목표 입력 계산은 후속 검색 수를 고정한 건수 환산이며 예측 효과나 실험 표본 수가 아닙니다. 목표값은 별도로 정해야 합니다.';$('#slide-script .speaker-notes').append(extra);}if(selectors[0].startsWith('#template-'))$('#slide-script').replaceChildren();if(selectors.includes('#template-segments'))$('#slide-script').replaceChildren(cloneForSlide('#segment-presentation-notes'));if(selectors.includes('#template-after-search'))$('#slide-script').replaceChildren(cloneForSlide('#after-search-notes'));if(selectors.includes('#template-card-h'))$('#slide-script').replaceChildren(cloneForSlide('#card-h-notes'));if(selectors.includes('#template-data-summary'))$('#slide-script').replaceChildren(cloneForSlide('#data-summary-notes'));if(selectors.includes('#template-data-limits'))$('#slide-script').replaceChildren(cloneForSlide('#data-limits-notes'));for(const key of ['topic','problem','overview','funnel','intents','proposals','decisions']){if(selectors.includes('#template-story-'+key))$('#slide-script').replaceChildren(cloneForSlide('#story-'+key+'-notes'));}for(let h=1;h<=3;h++){if(selectors.includes('#template-hypothesis-'+h))$('#slide-script').replaceChildren(cloneForSlide('#hypothesis-'+h+'-notes'));}collapseSlideScript();$('#prev-slide').disabled=slideIndex===0;$('#next-slide').disabled=slideIndex===slides.length-1;window.scrollTo(0,0);}
...
{terms:['유의성','신뢰구간'],label:'유의성·신뢰구간',text:'통계적 판단과 추정의 불확실성 범위. 현재 A/B 결과는 미산출.'},
