...
if __name__ == "__main__":
    ...
    parser.add_argument("--check", action="store_true", help="Fail if 2팀발표자료_0908_04.html differs from sources")
    args = parser.parse_args()
    output = ROOT / "2팀발표자료_0908_04.html"
    content = render()
    if args.check:
        if not output.exists() or output.read_text(encoding="utf-8") != content:
            sys.exit("2팀발표자료_0908_04.html is stale. Run scripts/build.py and commit the rebuilt file.")
        print("PASS: 2팀발표자료_0908_04.html matches source files")
    else:
