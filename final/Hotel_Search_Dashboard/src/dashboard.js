...
function page(i){document.querySelectorAll('.page').forEach(e=>e.hidden=e.id!=='page-'+i);document.querySelectorAll('nav button').forEach(e=>{const active=+e.dataset.page===i;e.setAttribute('aria-selected',active);e.tabIndex=active?0:-1});$('.slicers').hidden=i===4||i===1;$('#slide-position').textContent=([4,0,1,2,3].indexOf(i)+1)+' / 5';window.scrollTo(0,0)}
...
const intents=[['예산 유연형','A-1 가격 범위 확대','H1/H2'],['옵션 유연형','A-1 필요한 옵션 수 감소','H1/H2'],['위치 유연형','D-1 인접지역 제안','H5'],['조건 고수형','F-1 반복 안내','추가 검토 · 문서 가설 제외'],['표현 수정형','D-2 연관 검색어 제안','추가 검토 · 발표 가설 제외'],['빠른 해결형','C-1 단계별 안내','추가 검토 · 발표 가설 제외']];const savedIntent=$('#intent').value;$('#intent').innerHTML=intents.map((x,i)=>`<option value="${i}">${x[0]}</option>`).join('');function intent(){let x=intents[+$('#intent').value];$('#intent-content').innerHTML=`<div class="divider"></div><h3>${x[1]}</h3><p>연결 가설: ${x[2]}</p><p class="meta">문서의 개선안 매핑입니다. 이 경량 DB에는 의도군별 A/B 배정값이 포함되어 있지 않습니다.</p>`}$('#intent').value=savedIntent||'0';$('#intent').onchange=intent;intent();
