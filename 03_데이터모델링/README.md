...
> 원본을 바로 집계하지 말고 [`데이터품질/README.md`](데이터품질/README.md)와
> [`데이터품질/data_quality_manifest.json`](데이터품질/data_quality_manifest.json)의
> 품질 플래그 처리 규칙을 먼저 적용한다.

현재 분석 기준은 `2026-09-03_v03`이다. `search`와 `event` CSV 및 SQLite 테이블에
분석용 품질 플래그가 직접 포함되어 있다. 이전 v02는
`이전버전/데이터셋/2026-09-03_v02/`에 원천값 그대로 보존한다.
...
SQLite 분석은 [`데이터품질/analysis_guardrails.sql`](데이터품질/analysis_guardrails.sql)을
실행해 원본 DB를 변경하지 않는 TEMP VIEW를 사용할 수 있다.

변경 내용과 사용 방법은
[`데이터품질/호텔검색_데이터품질갱신안내_20260903_v01_현행본.md`](데이터품질/호텔검색_데이터품질갱신안내_20260903_v01_현행본.md)를
먼저 확인한다. AI 분석은
[`데이터품질/호텔검색_AI데이터분석지침_20260903_v01_현행본.md`](데이터품질/호텔검색_AI데이터분석지침_20260903_v01_현행본.md)도
함께 적용한다.
