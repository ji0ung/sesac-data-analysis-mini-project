...
def event_logic(data: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    add("session_end_time_before_last_SEARCH", "session with non-null session_end_time and SEARCH", len(common_s),
        int((declared.loc[common_s] < last_search.loc[common_s]).sum()),
        excluded=len(declared.index.symmetric_difference(last_search.index)),
        note="earliest declared end compared with last SEARCH; sessions missing either comparator are excluded")
